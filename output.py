...
def dict_generator(generator):
    ...
    for key, data in generator:
        dict_g[key] = data
    return dict_g
def ark_file(feat,dnn,fold="source_filter"):
    base_path = "/data/ac1zy/pytorch-kaldi-new/pytorch-kaldi/exp/"+fold+"/"+feat+"/exp_files"
    if fold=="source_filter":        
        file = os.path.join(base_path,"forward_test_ep49_ck04_out_"+dnn+".ark")
    else:
        file = os.path.join(base_path,"forward_test_ep49_ck4_out_"+dnn+".ark")
...
dnn31 = ark_file("fd1_vt_exc_sp","dnn31")
data,label,label_dys=concat_data(dnn31)
df31_vt_exc_sp = plot_2d(data)
with open('pickle/vt_exc_sp_dnn31.pickle', 'wb') as handle:
    pickle.dump(df31_vt_exc_sp, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
    
dnn32 = ark_file("fd1_vt_exc_sp","dnn32")
data,label,label_dys=concat_data(dnn32)
df32_vt_exc_sp = plot_2d(data)
with open('pickle/vt_exc_sp_dnn32.pickle', 'wb') as handle:
    pickle.dump(df32_vt_exc_sp, handle, protocol=pickle.HIGHEST_PROTOCOL)

dnn4 = ark_file("fd1_vt_exc_sp","dnn4")
data,label,label_dys=concat_data(dnn4)
df4_vt_exc_sp = plot_2d(data)
with open('pickle/vt_exc_sp_dnn4.pickle', 'wb') as handle:
    pickle.dump(df4_vt_exc_sp, handle, protocol=pickle.HIGHEST_PROTOCOL)
